...
resource.create.write.before((req, res, context) => {
    ...
    database.directories.findOne({
        ...
        if (!result) {
            send400(res, ['New directory must be child of existing directory']);
            context.stop();
            return;
        } else {
...
resource.update.write.before((req, res, context) => {
    ...
    if (req.body.directoryid && req.body.directoryid !== req.params.directoryid) {
    ...
    database.directories.findByPk(directoryid).then((directory) => {
        if (!directory) {
        ...
        if (isValidTopLevel.test(olddirectoryname) || isValidTopLevel.test(newdirectoryname)) {
            return send400(res, ['Cannot rename to/from top level directory']);
        }
        ...
        database.directories.findOne({
            ...
            if (!result) {
                return send400(res, ['New directory name must be child of existing directory', 'Groupname must exist']);
            }
...
resource.delete.write.before((req, res, context) => {
    ...
    database.directories.findOne({
        ...
        database.sequelize.query(
            ...
            res.status(200).send({ message: metadata.rowCount + ' rows deleted' });
        }, (error) => {
            debug(error);
            res.status(500).send({ message: 'internal error', errors: [error.name]});
        });
