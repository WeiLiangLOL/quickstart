...
class FileList extends React.Component {
    ...
    fileItem(groupname, _groupname) {
        return (
            ...
            aria-labelledby={"grouplist-" + _groupname} >
                <h4>{groupname}</h4>
                <a href="#" className="refreshSymbol" onClick={() => loadJSTree(groupname)}>&nbsp;</a>
                <a href="#" className="pinSymbol" onClick={() => togglePin("filelist-" + _groupname)}>&nbsp;&nbsp;</a>
    ...
    render() {
        return (
            <React.Fragment>
                {
                    this.props.groupArray.map(({groupname, _groupname}) => this.fileItem(groupname, _groupname))
                }
            </React.Fragment>
        );
    }
