...
function displayMetaList(groupname, data) {
    var promise;
    if (data.node.id.substring(0,1) === 'f') { // file
        promise = myAjax.readRegularFile(data.node.id.replace(/^f/, '')).then((file) => {
    ...
    promise.then((arr) => {
        ...
        ReactDOM.render(
            <MetaList type={arr[0]} name={arr[1]} id={arr[2]} data={arr[3]} user_acls={arr[4]} dir_acls={arr[5]} />,
